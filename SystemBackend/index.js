...
const MerchantFeedback = require("./models/MerchantFeedback");
...
const storage = multer.diskStorage({
  ...
  // Direct app.post usage
app.post("/merchantfeedback", async (req, res) => {
  try {
    const newFeedback = new MerchantFeedback(req.body);
...
// GET: Retrieve all merchant feedback
app.get("/getmerchantfeedback", async (req, res) => {
  try {
    const feedbacks = await MerchantFeedback.find().sort({ enrolledOn: -1 });
